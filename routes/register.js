...
router.get("/createdbtable", (req, res) => {
  let sql =
    "create table user(id int primary key auto_increment,name varchar(20), email varchar(100), password varchar(60))";
...
router.post("/create", function(req, res, next) {
  ...
  req.checkBody("usn");
  req
    .checkBody("re-password", "Password Do Not Match.")
  ...
  if (error) {
    ...
    bcrypt.hash(pass, null, null, function(err, hash) {
      db.query(
        "insert into user(fname,lname,usn,email,password) values(?,?,?,?,?)",
        [fname, lname, usn, email, hash],
